        ...
        for (const item of body.studentPromotions) {
          try {
            const student = await tx.student.findFirst({
              ...
              include: { user: { select: { name: true } }, class: { select: { classNumber: true, section: true } } },
            });
            ...
            let finalClassId: number | null = body.fromClassId; // default for TC/DROPPED/LEFT_SCHOOL history

            if (item.status === "PROMOTED" || item.status === "FAILED" || item.status === "DETAINED") {
              let toClassId: number;

              if (item.status === "PROMOTED") {
                toClassId = body.toClassId;
              } else {
                // Repeat the same grade — find (or require) that grade's
                // class in the NEW session. Previously this either linked
                // to fromClassId (the OLD session's class — wrong once
                // fromSession !== toSession) or, for DETAINED, did nothing
                // at all, leaving the student pointing at a class from a
                // session that's ending.
                if (!student.class) { result.errors.push(`${student.user.name}: has no current class to repeat.`); continue; }
                const repeatClass = await tx.class.findFirst({
                  where: { schoolId, classNumber: student.class.classNumber, section: student.class.section, academicYear: body.toSession },
                });
                if (!repeatClass) {
                  result.errors.push(`${student.user.name}: Class ${student.class.classNumber}-${student.class.section} doesn't exist yet for ${body.toSession} — create it first.`);
                  continue;
                }
